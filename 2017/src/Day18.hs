...
import           Prelude                    (error, init, last)
import           Protolude
...
doComputerStep2 comp
  ...
  | otherwise =
    case op' of
      Nothing -> Left (putStatusDone comp)
      Just _  -> comp & pc %~ (+ 1) & action op
  where
    r = readVal (comp ^. register)
    op' = view program comp Map.!? view pc comp
    Just op = op'
    ...
    action (Mul (Reg a) b) = Right . (register . at a %~ map (* r b))
    action (Mod (Reg a) b) = Right . (register . at a %~ map (`mod` r b))
    action (Rcv (Reg x)) = Left . (status .~ Receiving x)
    action (Jgz x y) =
      Right .
      if r x <= 0
        then identity
...
communicateab :: Computer -> Computer -> (Computer, Computer)
communicateab a b
...
communicateba :: Computer -> Computer -> (Computer, Computer)
communicateba a b
  | null $ a ^. output = (a, b)
...
popMsg :: Computer -> Computer
popMsg = output %~ init

putStatusRunning :: Computer -> Computer
putStatusRunning = status .~ Running

putStatusDone :: Computer -> Computer
putStatusDone = status .~ Done
...
main = do
  ...
  print $ choochoo begin ^. output ^?! ix 0
  let patchP id = (register . at 'p') ?~ id
  let !res = choochoos (begin & patchP 0, begin & patchP 1)
  let (aa, bb) = res
  bb ^. sendCount & print
