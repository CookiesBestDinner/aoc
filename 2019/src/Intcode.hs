...
-- for day 23 which can read without there existing input
runM :: Monad m => (Int -> m ()) -> m Int -> Comp -> m ()
runM send recv comp = do
  ...
  readInput <- if i == 3 then Just <$> recv else pure Nothing
  when (i == 4) $ send ra
  when (i /= 99) $ runM send recv $ comp & case i of
